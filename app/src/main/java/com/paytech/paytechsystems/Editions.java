...
import com.paytech.paytechsystems.adapter.EditionAdapter;
...
public class Editions extends AppCompatActivity  implements EditionAdapter.EditionAdapterListener, ActionBar.OnNavigationListener, SwipeRefreshLayout.OnRefreshListener{
        private static final String TAG = Editions.class.getSimpleName();
  ///private List<User> usersList = new ArrayList<>();
    private SwipeRefreshLayout swipeRefreshLayout;
    private RecyclerView recyclerView;
    private EditionAdapter mAdapter;
    private List<Edition> stdList = new ArrayList<>();;
...
//private static final String URL = "https://api.androidhive.info/json/contacts.json";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter = new EditionAdapter(this, stdList, this);
...
//        });
   ...
   /**
    ...
    private void fetchUsers() {
        ...
        Intent i = getIntent();
       final String course = i.getExtras().getString("course");
        swipeRefreshLayout.setRefreshing(true);
        final JsonArrayRequest request = new JsonArrayRequest(Config.URL +"editions.php?offset=",
                    ...
                    public void onResponse(JSONArray response) {
                        ...
                        for (int i = 0; i < response.length(); i++) {
                            try {
                                ...
                                Integer id = j.getInt("editionid");
                                String course = j.getString("course");
                                String edition = j.getString("edition");
                                String editionid = j.getString("editionid");
                                String fees = "1000";//j.getString("fee");
                                db.saveEdition(id, course, edition, fees, 1);
                                //Toast.makeText(getApplicationContext(), "Edition added to SQlite" +edition, Toast.LENGTH_LONG).show();
                        ...
                        List<Edition> items = new Gson().fromJson(response.toString(), new TypeToken<List<Edition>>(){}.getType());
 
                        // adding contacts to users list
                        stdList.clear();
                        stdList.addAll(items);
                        //stdList.addAll(db.getEdition(course));
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                stdList.clear();
                //stdList.addAll(db.getEdition(course));
                stdList.addAll(db.getAllEditions());
                mAdapter.notifyDataSetChanged();
    ...
    @Override
    public void onEditionSelected(Edition i) {

        Toast.makeText(getApplicationContext(), "Selected: " + i.getEdition() , Toast.LENGTH_SHORT).show();
    }
